...
-- Rodar DEPOIS do supabase-schema.sql
-- Substitui policies basicas por admin-only
-- ============================================

-- 1. TABELA DE ADMINS + FUNCAO DE VERIFICACAO
...
-- 2. SUBSTITUIR POLICIES BASICAS POR ADMIN-ONLY
...
-- 3. RPC: CONSULTA PUBLICA DE HORARIOS (sem expor dados do cliente)
...
-- 4. RPC: CRIAR AGENDAMENTO PUBLICO (com validacao server-side)
...
BEGIN
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION 'Barbeiro nao atende nesta data.';
    END IF;
    ...
    INSERT INTO public.appointments (
        barber_id, service_ids, service_names,
        appointment_date, appointment_time,
        client_name, client_phone, obs,
        status, total_price, total_duration
    ) VALUES (
        p_barber_id, v_service_ids, v_service_names,
        p_appointment_date, p_appointment_time,
        trim(p_client_name), trim(p_client_phone), p_obs,
        'confirmed', v_total_price, v_total_duration
    )
